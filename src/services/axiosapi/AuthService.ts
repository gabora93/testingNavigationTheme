...
import { LOGIN, SIGNUP, GETTOKEN, ISACTIVATED, ACCEPT_T_C } from '../../constants';
...
const AuthService = () => {
    ...
    const doLoginWithSSL = (body) => {
        return axiosInstance.post(TESTWITHSSL, {
            userName: body.userName,
            password: body.password,
            api_token: body.api_token
        });
    ...
    const postDataWithUserToken = async (url, data) => {
        const jwtToken = await getFromSecureStore("jwt");
        // ... rest of the postDataWithUserToken function
    };

    const doSignUp = (body) => {
        let res = axiosInstance.post(SIGNUP, {
    ...
    const isUserVerified = (body) => {
        let res = axiosInstance.post(ISACTIVATED, {
            userID: body.userID
        });
        return res;
    };

    const acceptTermsAndConditions = (body) => {
        let res = axiosInstance.post(ACCEPT_T_C, {
            userName: body.userName
        });
        return res;
    };

    return { doLogin, doSignUp, isUserVerified, acceptTermsAndConditions, getUserToken2}
